...
| **Throughput** | Real-time upload (↑) and download (↓) speeds, with a ring showing usage relative to your [connection's capacity](/user-guide/settings/monitors-settings/bandwidth-monitor/#bandwidth). |
...
### Reading the latency timeline

Every segment shares the same latency timeline. Once you can read it in one segment, you can read it in all of them:

<div class="img-md">

![Annotated latency timeline showing the status indicator, timeline, latency figure, packet loss and jitter warnings, and particle effects](./internet-dashboard-images/internet-dashboard-detailed-latency-timeline.png)

</div>

| Element | Description |
|---------|-------------|
| **Status** | A coloured dot and label (**OK**, **Light congestion**, **Heavy congestion** or **Error**) summarising the latency status of this hop. |
| **Timeline** | A bar graph of recent latency readings, newest on the right. Each bar is colour-coded from green (Good) to red (Error) so you can spot recent trends and spikes at a glance. |
| **Latency** | The current latency to the next hop (e.g. 17.5 ms). |
| **Packet Loss Warning** | A drop icon that appears beside the latency figure when packet loss exceeds 1% on this hop. Mouse over the hop to see more detail. |
| **Jitter Warning** | A waveform icon that appears when latency is varying significantly — high jitter, which can disrupt video calls and gaming. Mouse over the hop to see more detail. |
| **Particle effects** | An animated flow beneath the timeline that visualises live traffic across the hop — color indicates congestion, follow the same scheme as the graph. |
...
![This Mac segment showing status, latency graph, the connected access point name (TV Room 6G) above the Mac icon, connection selector, and throughput dials](./internet-dashboard-images/internet-dashboard-detailed-this-mac-section.png)
...
| **Access Point** | Above the Mac icon, the name of the access point you're connected to (e.g. **TV Room 6G**), shown when one has been configured. See [Access Point names](#access-point-names). |
...
![This Mac popover showing Tx rate, signal, channel, security, the named access point, and network identifiers for a Wi-Fi connection](./internet-dashboard-images/internet-dashboard-detailed-this-mac-popover.png)
...
| **Access Point** | The access point your Mac is connected to, shown by name with its manufacturer's icon. Click the pencil to rename it — see [Access Point names](#access-point-names). |
...
:::note
The fields shown depend on your connection type. Wi-Fi connections show signal, channel, security, access point, and BSSID; a wired Ethernet connection shows the relevant subset.
:::

#### Access Point names

The **Access Point** field names the specific access point your Mac is associated with, instead of leaving you to read a raw hardware address. PeakHour resolves it from the connection's **BSSID** (the access point's MAC address) and shows the access point manufacturer's icon when it can identify the hardware.

To give an access point a friendlier label, click the pencil beside its name and type your own (for example, *TV Room 6G*). PeakHour remembers the name against that BSSID and reuses it wherever that access point is shown — including the [Wi-Fi menu bar item](/user-guide/settings/menu-bar/#wi-fi-menu-bar-item).

This is most useful on mesh or multi-access-point networks: as your Mac roams between access points, the **Access Point** field updates to whichever one is currently serving it — so you can tell at a glance which node you're on, and notice when you've roamed onto a weaker or more distant one.
...
![Router segment showing status, latency graphs, gateway label and throughput dials](./internet-dashboard-images/internet-dashboard-detailed-router.png)
...
PeakHour displays the detected Internet Gateway in this slot, recognising many router brands and showing the manufacturer's badge (for example, the UniFi logo) on the device icon. For more on configuring a gateway, see [First Time Setup](/user-guide/first-time-setup/) and [Settings → Bandwidth Monitor](/user-guide/settings/monitors-settings/bandwidth-monitor/).
...
| **Gateway label** | The name (e.g. **UniFi Dream Machine**) of the [Bandwidth Monitor](/user-guide/settings/monitors-settings/bandwidth-monitor/) representing your Internet Gateway. It's normally detected automatically, but you can designate it manually in Settings. If no Bandwidth Monitor is detected or designated, the pill shows the default gateway's IP address instead, and the throughput dials stay empty. Click it to open the router information popover. |
| **Throughput** | The two dials show real-time download (↓) and upload (↑) throughput reported by the router. Each dial fills relative to the gateway's configured [Bandwidth settings](/user-guide/settings/monitors-settings/bandwidth-monitor/#bandwidth). |
...
| **Max Throughput** | The two dials show the maximum download (↓) and upload (↑) speeds measured by the most recent [Network Quality test](/user-guide/settings/dashboard/#network-quality) — not real-time throughput. |
...
:::note
The **Max Throughput** dials are only populated when Network Quality measurement is enabled. Enable it under [Internet Dashboard settings → Network Quality](/user-guide/settings/dashboard/#network-quality).
:::
...
:::note
The Quality Report is only available when periodic [Network Quality measurement](/user-guide/settings/dashboard/#network-quality) is enabled. Clicking it displays the latest test results rather than running a new test — to re-run the test on demand, click the **Refresh** button at the top of the report.
:::
...
Click the segment to open a popover listing the **Internet Hosts** — the endpoints PeakHour uses for connectivity checks:

<div class="img-sm">

![Internet Hosts popover listing the monitored endpoints — Google, Zoom and Cloudflare — each with a status dot](./internet-dashboard-images/internet-dashboard-internet-popover.png)

</div>

PeakHour measures the final hop by reaching out to several well-known endpoints (for example, **Google**, **Zoom**, and **Cloudflare**) and combining the results. Each host shows a status dot indicating whether it's currently reachable, along with the hostname used for the check. You can change which hosts are monitored in [Settings → Dashboard](/user-guide/settings/dashboard/).
...
The AI Advisor re-evaluates your connection every 30–60 seconds rather than continuously, so its grade and analysis reflect the most recent assessment rather than a live, real-time value. The ring around the [AI Advisor Rating](#ai-advisor-rating) badge counts down the time until the next re-evaluation.
