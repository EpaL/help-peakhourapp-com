...
**Colors** — PeakHour uses color to indicate latency, blending from Normal (minimum latency) through Light Congestion to Heavy Congestion as a gradient. Optionally, the monitor's background changes to match.

| Color | Meaning |
|-------|---------|
| **Normal** | The base color for 'good' or optimum latency. The baseline minimum is set under [Latency](#latency). |
| **Light Congestion** | The color for slight congestion. |
| **Light Congestion Latency** | What counts as slight congestion. Default = Normal latency + 75 ms. |
| **Heavy Congestion** | The color for significant congestion. |
| **Heavy Congestion Latency** | What counts as significant congestion. Default = minimum + light-congestion latency + 125 ms. |
| **Packet Loss** | The color marking packet loss, shown as a grey mark rather than plotted as a series. |

**Background color indicates congestion** — when enabled, the details view's background blends with the current latency color.

## Latency

Adjusts what PeakHour treats as 'normal' (minimum) latency — the baseline for the Normal, Light Congestion, and Heavy Congestion colors.
