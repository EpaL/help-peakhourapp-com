...
description: "Latency monitors ping a target or host (or set of targets/hosts) to determine the latency to that host."
sidebar:
  order: 19
status: needs-review
---
...
Latency monitors ping a target or host (or set of targets/hosts) to determine the latency to that host. For more information on Latency Monitors, see [Configuration Assistant: Latency](/user-guide/configuration-assistant/configuration-assistant-latency/) .

The Target tab provides a summary of the Latency target, and lets you control its visibility.
