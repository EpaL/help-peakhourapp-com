---
title: "Latency options"
description: "Options on this tab allow you to adjust what PeakHour considers 'normal' latency for what you are monitoring."
sidebar:
  order: 21
status: needs-review
---



![CleanShot 2024 05 25 at 13.50.08@2x 20240525 175013](./latency-options-images/cleanshot-2024-05-25-at-13-50-08-2x-20240525-175013.png)

Options on this tab allow you to adjust what PeakHour considers 'normal' latency for what you are monitoring.

By default, PeakHour will automatically detect and measure the minimum latency. By default, this minimum latency will reset every time PeakHour is restarted, or monitors are reset. This minimum (or normal) latency is used a baseline for Normal, Light Congestion and Heavy Congestion states and colors, configured in [Display & Colors \[Latency\]](/user-guide/settings/monitors-settings/latency-monitor/display-colors-latency/) . These are visualized in the color of the text, background (optional) and graph of the monitor.

**Latency**

This slider lets you adjust what is considered the minimum latency for the connection. By default, this is calculated automatically.

The minimum latency is used to calculate the colors used when displaying the latency and latency graph.

**Automatic: Reset on start**

If Latency is set to Automatic, this will reset the minimum latency every time PeakHour starts, or monitors are reset. This is on by default.

**Show minimum latency on graph**

Turns the minimum latency line display in the graph on or off.

**With labels**

Include the minimum latency value on the line.

