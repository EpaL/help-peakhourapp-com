...
- **Usage isn't counted against it.** When Usage is set to Automatic, PeakHour won't count data against a router you haven't adopted, so passing through a café or a hotel never adds to your usage totals.
...
- **One router per network.** PeakHour shows at most one detected router at a time, and prefers the router your Mac uses to reach the internet. On a mesh Wi-Fi system, that means you'll see the main router rather than every node.
